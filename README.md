...
В code context сначала строится безопасный кандидат, который не меняет существующую ASCII-пунктуацию. Дополнительный кандидат начинается с безопасного результата и исправляет только подтверждённые syntax-позиции: wrong-layout кавычки и terminator после закрывающей скобки или подтверждённой закрывающей кавычки. Обычный кириллический текст внутри корректных ASCII-кавычек не затрагивается. При сомнении остаётся безопасный вариант. Подтверждённый VS Code Integrated Terminal игнорируется до Copy и изменения буфера обмена.
...
Исходный код KeyFlip доступен для проверки, а приложение работает полностью локально. Неподписанный EXE может вызвать предупреждение Windows SmartScreen или антивируса. Не отключайте защиту и не добавляйте неизвестные файлы в исключения: скачивайте KeyFlip только из официального GitHub Release, сверяйте SHA-256 или собирайте приложение самостоятельно из исходников.
